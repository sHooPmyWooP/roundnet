...
def calculate_win_rate(team_id: str) -> float:
    ...
    games = get_games()
    team_games = [
        g for g in games if g["team_a_id"] == team_id or g["team_b_id"] == team_id
    ]
...
def get_team_performance_trends(team_id: str, window_size: int = 5) -> pd.DataFrame:
    ...
    if len(df) > 0:
        df["rolling_win_rate"] = (
            df["won"].rolling(window=window_size, min_periods=1).mean()
        )
        df["rolling_avg_points"] = (
            df["points_for"].rolling(window=window_size, min_periods=1).mean()
        )
...
def get_head_to_head_record(team_a_id: str, team_b_id: str) -> dict[str, Any]:
    ...
    h2h_games = [
        g
        for g in games
        if (g["team_a_id"] == team_a_id and g["team_b_id"] == team_b_id)
        or (g["team_a_id"] == team_b_id and g["team_b_id"] == team_a_id)
    ]
